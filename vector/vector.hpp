...
namespace ft
{
	...
	public:
		...
		template<class In> //std::enable_if<!std::is_integral<In>::value> >
		...
		insert(const_iterator pos, In first, In last) {
 			...
 			for (size_type i = _size + count - 1; i > index + count - 1; --i) {
 			  _data[i] = _data[i - count]; // in c++ 11 they are using std::move or static_cast<T&&> but we are not allowed! aq
 			}
